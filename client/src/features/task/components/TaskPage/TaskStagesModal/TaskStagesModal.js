...
import { stageColorClasses, stageColorOptions } from "../../../utils/stageColors";

export default function TaskStagesModal({ open, setOpen }) {
  ...
  const handleSubmit = (e) => {
    ...
    createTaskStage(
      {
        name: trimmed.toLocaleLowerCase("tr").replace(/\s+/g, "-"),
        title: trimmed,
        color,
      },
      {
        ...
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-white/10 p-6">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Kanban panosundaki aşamaları (durumları) yönetin.
            </p>
          </div>
        ...
        <div className="scrollbar-thin max-h-[60vh] overflow-y-auto p-6">
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <Button
              ...
              text={isCreating ? "Ekleniyor..." : "Aşama Ekle"}
              className="w-full rounded-xl px-5 py-3 disabled:cursor-not-allowed disabled:opacity-60"
            />
          </form>
            ...
            {stages.map((stage) => (
              ...
              >
                <span className="flex items-center gap-2 text-sm font-medium text-gray-800 dark:text-gray-200">
                  <span
                    className={`h-2 w-2 rounded-full ${
                      stageColorClasses[stage.color]?.dot || "bg-gray-400"
                    }`}
                  />
                  {stage.title}
                </span>
                ...
                <button
                  type="button"
                  disabled={isDeleting}
                  onClick={() => deleteTaskStage(stage.id)}
